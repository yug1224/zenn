...
さらに、oxlintの組み込みhooksルールと `eslint-plugin-react-hooks` v7をjsPluginとして読み込む **二重ガード** を敷くことで、Compilerが想定しない書き方をlint段階で検知できるようにしています。

この記事では、annotationモードの設定方法、`"use memo"` / `"use no memo"` の判断基準、oxlintによる二重ガードの仕組み、そして8,000超ファイルへの段階適用戦略をまとめます。
...
| ライブラリ                           | 理由                                                       |
| ------------------------------------ | ---------------------------------------------------------- |
| `useReactTable`（TanStack Table v8） | テーブル設定オブジェクトの参照がCompilerのメモ化と衝突する |
| `react-hook-form`                    | フォーム状態管理の内部実装がCompilerの前提と合わない       |
| `useInfiniteQuery`（TanStack Query） | 無限スクロールのページパラメータ管理がCompilerと非互換     |
...
Compiler導入の初期に痛感したのは、従来の `rules-of-hooks` / `exhaustive-deps` だけではCompiler非互換のコードをすり抜けてしまうことでした。CIが通ったのにランタイムで壊れる——この事故を防ぐために、oxlintで2系統のhooksルールを運用しています。
...
新規作成するコンポーネント・フックにはすべて `"use memo";` を付ける運用をチームで合意しました。既存ファイルには手を加えません。AGENTS.mdにルールを明記し、AIエージェント（Cursor / Claude Code）も同じ基準で自動付与するようにしています。最初の2週間で新規ファイル約40件に適用しましたが、この段階ではCompiler起因のトラブルはゼロでした。

### Phase 2: 機能単位で既存ファイルに拡大

リファクタリングや機能追加のタイミングで、触ったファイルに `"use memo";` を付与します。`pnpm lint` を実行し、Compiler由来のlintエラーが出たら修正するか `"use no memo";` にフォールバックします。正直、想定外だったのはreact-hook-form周りで `"use no memo";` が必要なファイルが予想以上に多かったことです。判断基準テーブルはこのPhaseで何度か更新しました。

### Phase 3: ディレクトリ単位の一括適用

安定した領域（ユーティリティ、共通コンポーネント等）からcodemodで `"use memo";` を一括付与し、CIで回帰を検証する予定です。

### Phase 4: `"all"` モードへの移行

`"use no memo";` のファイルが十分に減ったタイミングで `compilationMode` を `"all"` に切り替え、`"use no memo";` だけをオプトアウトとして残します。

:::message
Phase 3以降は今後の計画であり、本記事執筆時点ではPhase 2を進行中です。
:::
...
ファイル先頭で `react-hooks-js/*` を列挙する方式は、どの行が本当に違反しているのか分からなくなるため禁止しています。これは地味に引っかかりやすいポイントなので、チーム内で最初に共有しておくと良いと思います。
...
3. **oxlint の二重ガード**（組み込み `react-hooks/*` + jsPlugin `react-hooks-js/*`）で、従来のhooks違反とCompiler非互換の両方をlint段階で検知する
4. **段階適用**（新規→機能単位→ディレクトリ単位→全適用）で、8,000超ファイルへの展開を安全に進める
