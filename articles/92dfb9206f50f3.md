...
Compiler導入の初期に痛い目を見ました。CIはオールグリーンなのに、ステージング環境でテーブルのソートが効かない——原因はCompiler非互換のコードが従来の `rules-of-hooks` / `exhaustive-deps` をすり抜けていたことでした。React公式の [`eslint-plugin-react-hooks`](https://react.dev/reference/eslint-plugin-react-hooks) v7にはCompiler由来の検出ルール（`incompatible-library` / `unsupported-syntax` 等）が統合されており、OxlintにはESLintプラグインをそのまま読み込める[jsPlugin](https://oxc.rs/docs/guide/usage/linter/js-plugins)という仕組みがあるので、これを活用してランタイムで壊れる事故を防いでいます。
