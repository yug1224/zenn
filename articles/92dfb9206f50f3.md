...
emoji: "🦺"
type: "tech"
topics: ["react", "reactcompiler", "oxlint", "vite", "frontend"]
published: true
publication_name: "dress_code"
published_at: "2026-05-19 19:00"
---

![React CompilerをannotationモードとOxlintで漸進的に導入する](/images/92dfb9206f50f3/Gemini_Generated_Image_jwbt3sjwbt3sjwbt.png)
_React CompilerをannotationモードとOxlintで漸進的に導入する_
...
Viteプロジェクトでは `vite-plugin-babel` を経由して `babel-plugin-react-compiler` を適用します。[React公式ドキュメント](https://react.dev/learn/react-compiler/installation#vite)では `@vitejs/plugin-react` の `babel` オプションを使う方法が第一に紹介されていますが、Dress Codeでは `@vitejs/plugin-react-swc` を使っているため、Babelプラグインを別途読み込む `vite-plugin-babel` を採用しています。
...
Compiler導入の初期に痛い目を見ました。CIは通っているのに、実際に動かすとテーブルのソートが効かない。前述のlintだけでは、この手の不具合を事前に検知できませんでした。React公式の [`eslint-plugin-react-hooks`](https://react.dev/reference/eslint-plugin-react-hooks) v7にはCompiler由来の検出ルール（`incompatible-library` / `unsupported-syntax` 等）が統合されており、OxlintにはESLintプラグインをそのまま読み込める[jsPlugin](https://oxc.rs/docs/guide/usage/linter/js-plugins)という仕組みがあるので、これを活用して、CI 上で主要な非互換を先に検知し、既知パターンのランタイム不具合を減らしています。
...
flowchart TB
  ...
  subgraph js [jsPlugin: eslint-plugin-react-hooks v7]
    ...
    OtherRules["react-hooks-js/その他 recommended ルール"]
  end
